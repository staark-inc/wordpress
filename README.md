...
- **S-Hub Inbox** — Forms & Booking (part of Staark Hub, own admin menu; rename with the `staark_hub_inbox_label` filter) — Inbox for every form request, bookings with statuses (pending, confirmed, declined, cancelled, completed) and a day calendar, per-form recipients, customer emails from editable templates (received, confirmed, declined, cancelled, replies) and admin notifications (menu badge and admin bar bell). Booking forms can send structured `booking_type`, `booking_date`, `booking_end_date`, `booking_time`, `booking_guests` and `booking_item` fields next to the normal `[staark_contact_form]` fields.
