<?php
/**
 * Contact shortcodes for S-Hub Bygg.
 *
 * Contact details are entered once in Staark Hub (First Install / SEO) and
 * reused in the header, quote section, footer and mobile quote bar.
 *
 * [bygg_contact field="phone" link="1" fallback="08-000 00 00"]
 ...
 */
function staark_bygg_contact_details(): array
{
    static $details = null;
 ...
 */
function staark_bygg_contact_shortcode($atts = []): string
{
    $atts = shortcode_atts(
        [
            'field' => 'phone',
    ...
    $details = staark_bygg_contact_details();
    ...
    if ($value === '') {
        ...
        if ($hint !== '' && current_user_can('edit_pages')) {
            return '<span class="bygg-contact bygg-contact--missing">' . esc_html($hint) . '</span>';
        }
    ...
    if (! $link) {
        return '<span class="bygg-contact bygg-contact--' . esc_attr($field) . '">' . esc_html($text) . '</span>';
    }
    ...
    return sprintf(
        '<a class="bygg-contact bygg-contact--%1$s" href="%2$s"%3$s>%4$s</a>',
        esc_attr($field),
...
add_shortcode('bygg_contact', 'staark_bygg_contact_shortcode');

/*
 * Current year for footer copy without relying on the Hub.
 */
add_shortcode('bygg_year', static fn (): string => esc_html(wp_date('Y')));
