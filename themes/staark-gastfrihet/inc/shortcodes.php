<?php
/**
 * Contact shortcodes for S-Hub Gästfrihet.
 *
 ...
 *
 * [gast_contact field="phone" link="1" fallback="08-000 00 00"]
 ...
 */
function staark_gast_contact_details(): array
{
    static $details = null;
 ...
 */
function staark_gast_contact_shortcode($atts = []): string
{
    $atts = shortcode_atts(
        [
            'field' => 'phone',
    ...
    $details = staark_gast_contact_details();
    ...
    if ($value === '') {
        ...
        if ($hint !== '' && current_user_can('edit_pages')) {
            return '<span class="gast-contact gast-contact--missing">' . esc_html($hint) . '</span>';
        }
    ...
    if (! $link) {
        return '<span class="gast-contact gast-contact--' . esc_attr($field) . '">' . esc_html($text) . '</span>';
    }
    ...
    return sprintf(
        '<a class="gast-contact gast-contact--%1$s" href="%2$s"%3$s>%4$s</a>',
        esc_attr($field),
...
add_shortcode('gast_contact', 'staark_gast_contact_shortcode');

/*
 * Current year for footer copy without relying on the Hub.
 */
add_shortcode('gast_year', static fn (): string => esc_html(wp_date('Y')));
