...
The homepage Contact pattern uses `[staark_contact_form form_id="staark-home"]` when Staark Hub is active. It links to `contact@staarkinc.com` either way. Submissions are stored and notifications are delivered according to **S-Hub Inbox → Notifications / Settings**; the theme does not send mail. Configure the recipient and SMTP transport there. Existing saved forms settings are intentionally preserved. On older Hub builds the theme loads the plugin's form stylesheet itself; newer Hub builds detect template forms through a filter.
