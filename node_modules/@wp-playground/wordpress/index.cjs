"use strict";Object.defineProperty(exports,Symbol.toStringTag,{value:"Module"});const f=require("@php-wasm/universal"),o=require("@php-wasm/util"),T=require("@wp-playground/common"),$=require("@php-wasm/logger");async function M(e){await e.writeFile("/internal/shared/mu-plugins/0-playground.php",`<?php
...
`;async function fe(e){await e.mkdir("/internal/shared/mu-plugins"),await e.writeFile("/internal/shared/auto_prepend_file.php",`<?php
...
`),await e.writeFile("/internal/shared/mu-plugins/1-auto-login.php",`<?php
		...
		`),await M(e),await e.writeFile("/internal/shared/preload/error-handler.php",`<?php
...
set_error_handler('_playground_error_handler');`)}const he=`
...
`;function H(e){return[/(^|[^\w$])((?:require|include)(?:_once)?)\s*\(\s*(['"])(\.\.\/[^'"]+)\3\s*\)/g,/(^|[^\w$])((?:require|include)(?:_once)?)\s*\(\s*(['"])(\.\/[^'"]+)\3\s*\)/g,/(^|[^\w$])((?:require|include)(?:_once)?)\s*\(\s*(['"])([a-z][\w-]*\.php)\3\s*\)/g,/(^|[^\w$])((?:require|include)(?:_once)?)\s+(['"])(\.\.\/[^'"]+)\3/g,/(^|[^\w$])((?:require|include)(?:_once)?)\s+(['"])(\.\/[^'"]+)\3/g,/(^|[^\w$])((?:require|include)(?:_once)?)\s+(['"])([a-z][\w-]*\.php)\3/g].reduce((i,n)=>i.replace(n,(s,r,a,l,_)=>`${r}${a}(${$e(_)})`),e)}function $e(e){let t=e,i=0;for(;t.startsWith("../");)i++,t=t.slice(3);for(;t.startsWith("./");)t=t.slice(2);let n="dirname(__FILE__)";for(let s=0;s<i;s++)n=`dirname(${n})`;return`${n} . '/${t}'`}async function ge(e,t){return X(e,t)?(await Te(e,t),await Ee(e,t),await we(e,t),await Le(e,t),await ye(e,t),await Pe(e,t),await Oe(e,t),!0):!1}async function me(e){return X(e,e.documentRoot)?(await e.run({code:`<?php
		...
		`,env:{DOCUMENT_ROOT:e.documentRoot}}),!0):!1}function X(e,t){const i=o.joinPaths(t,"b2config.php"),n=o.joinPaths(t,"b2-include/b2vars.php");return!e.fileExists(i)||!e.fileExists(n)?!1:e.readFileAsText(n).includes("$b2_version = '0.71'")}async function Te(e,t){for(const i of[o.joinPaths(t,"wp-includes"),o.joinPaths(t,"wp-content"),o.joinPaths(t,"wp-content/database")])e.isDir(i)||await e.mkdir(i)}async function Ee(e,t){const i=o.joinPaths(t,"wp-includes/version.php");await e.writeFile(i,`<?php
$wp_version = '0.71';
$wp_db_version = 71;
`);const n=o.joinPaths(t,"wp-config.php");e.fileExists(n)||await e.writeFile(n,"<?php require_once dirname(__FILE__) . '/b2config.php';");const s=o.joinPaths(t,"wp-load.php");e.fileExists(s)||await e.writeFile(s,`<?php
if (!defined('ABSPATH')) {
	define('ABSPATH', dirname(__FILE__) . '/');
}
require_once ABSPATH . 'b2config.php';
`);const r=o.joinPaths(t,"wp-admin/post.php");e.fileExists(r)||await e.writeFile(r,"<?php require_once dirname(__FILE__) . '/b2edit.php';")}async function we(e,t){const i=o.joinPaths(t,"b2config.php");let n=e.readFileAsText(i);const s=n;n.includes("pg_wp07_bootstrap")||(n=n.replace("<?php",`<?php
...
`)),n=n.replace(/\$siteurl\s*=\s*'http:\/\/example\.com';[^\n]*/,"$siteurl = defined('WP_SITEURL') ? WP_SITEURL : 'http://localhost'; // pg_wp07_siteurl").replace('$blogname = "my weblog";','$blogname = "My WordPress Website";').replace('$blogdescription = "babblings !";','$blogdescription = "Just another WordPress weblog";').replace("$admin_email = 'you@example.com';","$admin_email = 'admin@localhost.com';").replace("$abspath =  getenv('DOCUMENT_ROOT') . $relpath . '/';","$abspath = dirname(__FILE__) . '/';"),n!==s&&await e.writeFile(i,n)}async function Le(e,t){const i=o.joinPaths(t,"blog.header.php");let n=e.readFileAsText(i);const s=n;n=n.replace(`$where .= ' AND (post_status = "publish"';`,`$where .= " AND (post_status = 'publish'";`),n!==s&&await e.writeFile(i,n)}async function ye(e,t){const i=o.joinPaths(t,"b2-include/wp-db.php");let n=e.readFileAsText(i);const s=n;n=be(n),n=n.replace("$wpdb = new wpdb(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST);",`if (!isset($GLOBALS['wpdb'])) {
	$GLOBALS['wpdb'] = new wpdb(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST);
}
$wpdb = $GLOBALS['wpdb']; /* pg_wp07_preserve_sqlite_loader */`),n!==s&&await e.writeFile(i,n)}function be(e){const t=[];if(e.includes("function set_prefix")||t.push(`
	...
	function check_connection($allow_bail = true) {
		return true;
	}`),t.length===0)return e;const i=e.match(/^(\s*})\s*\n+(\$wpdb|if\s*\(\s*!\s*isset\(\s*\$GLOBALS\['wpdb'\]\s*\))/m);if(!i||i.index===void 0)throw new Error("WP 0.7 wpdb polyfill anchor not found; b2-include/wp-db.php layout changed");const n=`
	// Polyfills added by WordPress Playground for WP 0.7.
`+t.join(`
`)+`

`;return e.substring(0,i.index)+n+e.substring(i.index)}async function Pe(e,t){const i=o.joinPaths(t,"b2-include/b2template.functions.php");let n=e.readFileAsText(i);const s=n;n.includes("pg_wp07_apply_filters")||(n=n.replace(`function apply_filters($tag, $string) {
	global $b2_filter;
	...
	return $string;
}`),n!==s&&await e.writeFile(i,n))}async function Oe(e,t){await Ne(e,t),await Ae(e,t),await Ue(e,t),await Se(e,t)}async function Ne(e,t){const i=o.joinPaths(t,"b2login.php");let n=e.readFileAsText(i);const s=n;n.includes("pg_wp07_login_auto_login")||(n=n.replace("switch($action) {",`${j("pg_wp07_login_auto_login")}
switch($action) {`)),n.includes("pg_wp07_logout_guard")||(n=n.replace(`	setcookie('wordpressuser');
	...
	setcookie('wordpresspass', '', time() - 31536000, '/');
	setcookie('wordpressblogid', '', time() - 31536000, '/');
	setcookie('playground_auto_login_already_logged_out', '1', time() + 172800, '/'); /* pg_wp07_logout_guard */`),n=n.replace("header('Refresh: 0;url=b2login.php');","header('Refresh: 0;url=b2login.php?loggedout=1');").replace("header('Location: b2login.php');","header('Location: b2login.php?loggedout=1');")),n!==s&&await e.writeFile(i,n)}async function Ae(e,t){const i=o.joinPaths(t,"wp-admin");if(e.isDir(i))for(const n of e.listFiles(i)){if(!n.endsWith(".php"))continue;const s=o.joinPaths(i,n),r=e.readFileAsText(s),a=H(r);a!==r&&await e.writeFile(s,a)}}async function Ue(e,t){const i=o.joinPaths(t,"wp-admin/b2menutop.php");let n=e.readFileAsText(i);const s=n;n=n.replace('file("./b2menutop.txt")',"file(dirname(__FILE__) . '/b2menutop.txt')").replace('<a href="http://wordpress.org" rel="external"><span>WordPress</span></a>','<a href="#" rel="external"><span>WordPress</span></a>'),n!==s&&await e.writeFile(i,n)}async function Se(e,t){const i=o.joinPaths(t,"wp-admin/b2verifauth.php");let n=e.readFileAsText(i);const s=n;n.includes("pg_wp07_auto_login")||(n=n.replace("require_once('../b2config.php');",`require_once('../b2config.php');
${j("pg_wp07_auto_login")}`),n!==s&&await e.writeFile(i,n))}function j(e){return`
...
`}async function Re(e,t){const i=b(e,t);if(i===null)return;const n=parseFloat(i);if(!Number.isFinite(n)||n<5||n>=6.2)return;const s=o.joinPaths(t,"wp-includes/load.php");if(!e.fileExists(s))return;const r=e.readFileAsText(s),a=r.replace("extension_loaded( 'mysqli' )","function_exists( 'mysqli_connect' )");a!==r&&await e.writeFile(s,a)}function b(e,t){const i=o.joinPaths(t,"wp-includes/version.php");if(!e.fileExists(i))return null;const s=e.readFileAsText(i).match(/\$wp_version\s*=\s*['"]([^'"]+)['"]/);return s?s[1]:null}const xe=22527,P="E_ALL & ~8192 & ~2048";async function ve(e,t){if(await ge(e,t))return;await Be(e,t),await Xe(e,t),await je(e,t),await Qe(e,t),await ze(e,t),await ot(e,t),await Je(e,t),await st(e,t),await Fe(e,t),await De(e,t);const i=b(e,t);if(i===null)return;const n=parseFloat(i);Number.isFinite(n)&&(n<1.2&&(await He(e,t),await Me(e,t)),n<1.5&&await Ce(e,t),1.5<=n&&n<2&&await nt(e,t),n<2&&(await qe(e,t),await Ke(e,t)),2.1<=n&&n<2.3&&await Ge(e,t),n<2.5&&await et(e,t),n<2.8&&(await rt(e,t),await at(e,t)),2.9<=n&&n<3.6&&await ke(e,t),3.3<=n&&n<3.4&&await We(e,t),n>=4.7&&await Ie(e,t))}async function Ie(e,t){const i=o.joinPaths(t,"wp-content/themes");if(e.isDir(i))for(const n of e.listFiles(i)){const s=o.joinPaths(i,n,"searchform.php");e.fileExists(s)&&e.unlink(s)}}async function ke(e,t){const i=o.joinPaths(t,"wp-admin/includes/dashboard.php");if(e.fileExists(i)){let r=e.readFileAsText(i);if(r.includes("function wp_dashboard_primary()")&&!r.includes("/* pg_no_rss */")){for(const a of["wp_dashboard_primary","wp_dashboard_secondary","wp_dashboard_plugins"])r=r.replace(new RegExp(`function ${a}\\(\\)\\s*\\{`),`function ${a}() { /* pg_no_rss */ return;`);await e.writeFile(i,r)}}const n=o.joinPaths(t,"wp-admin/admin.php");if(e.fileExists(n)){let r=e.readFileAsText(n);r.includes("do_action('admin_init');")&&!r.includes("/* pg_admin_init_cleanup */")&&(r=r.replace("do_action('admin_init');",`/* pg_admin_init_cleanup */
...
do_action('admin_init');`),await e.writeFile(n,r))}const s=o.joinPaths(t,"wp-admin/includes/update.php");if(e.fileExists(s)){let r=e.readFileAsText(s);if(!r.includes("/* pg_admin_no_updates */")){for(const a of["wp_plugin_update_rows","wp_plugin_update_row","wp_theme_update_rows","wp_theme_update_row","wp_update_plugins","wp_update_themes"]){const l=new RegExp(`function ${a}\\s*\\([^)]*\\)\\s*\\{`);l.test(r)&&(r=r.replace(l,_=>_+" /* pg_admin_no_updates */ return;"))}await e.writeFile(s,r)}}for(const r of[o.joinPaths(t,"wp-includes/SimplePie/File.php"),o.joinPaths(t,"wp-includes/class-simplepie.php")]){if(!e.fileExists(r))continue;let a=e.readFileAsText(r);a.includes("function SimplePie_File(")&&!a.includes("/* pg_no_fetch */")&&(a=a.replace(/function SimplePie_File\([^)]*\)\s*\{/,l=>l+`
		...
		$this->success = false;
		return;`),await e.writeFile(r,a))}}async function De(e,t){const i=[["function wp_new_blog_notification","pg_no_blog_notification"],["function wp_install_maybe_enable_pretty_permalinks","pg_no_permalink_check"]],n=[o.joinPaths(t,"wp-admin/includes/upgrade.php"),o.joinPaths(t,"wp-admin/upgrade-functions.php")];for(const s of n){if(!e.fileExists(s))continue;let r=e.readFileAsText(s),a=!1;for(const[l,_]of i){if(r.includes(`/* ${_} */`))continue;const c=r.indexOf(l);if(c===-1)continue;const d=r.indexOf("{",c);d!==-1&&(r=r.substring(0,d+1)+` /* ${_} */ return;`+r.substring(d+1),a=!0)}a&&await e.writeFile(s,r)}}async function Fe(e,t){const i=o.joinPaths(t,"wp-load.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i);if(!n.includes("error_reporting(")||n.includes("~8192")&&n.includes("~2048"))return;const s=n.replace(/error_reporting\(([^)]+)\)/g,(r,a)=>`error_reporting((${a}) & ~8192 & ~2048)`);await e.writeFile(i,s)}async function Ce(e,t){const i=o.joinPaths(t,"wp-admin/menu.php");if(e.fileExists(i)){const r=e.readFileAsText(i);if(!r.includes("/* pg_wp10_logo_link */")){const a='<h1 id="wphead"><a href="http://wordpress.org" rel="external">WordPress</a></h1>';if(r.includes(a)){const l=r.replace(a,'<h1 id="wphead"><a href="#" rel="external">WordPress</a></h1> <!-- pg_wp10_logo_link -->');l!==r&&await e.writeFile(i,l)}}}const n=o.joinPaths(t,"wp-admin/admin-header.php");if(e.fileExists(n)){const r=e.readFileAsText(n);if(!r.includes("/* pg_wp12_logo_link */")){const a='<a href="http://wordpress.org" rel="external"',l="</a>",_=r.indexOf(a);if(_!==-1){const c=r.indexOf(l,_);if(c!==-1){const d=r.substring(0,_)+'<a href="#">WordPress</a><!-- pg_wp12_logo_link -->'+r.substring(c+l.length);d!==r&&await e.writeFile(n,d)}}}}const s=o.joinPaths(t,"wp-admin/admin-footer.php");if(e.fileExists(s)){const r=e.readFileAsText(s);if(!r.includes("/* pg_wp10_footer_link */")){const a=r.replace('<a href="http://wordpress.org">WordPress</a>',"WordPress<!-- pg_wp10_footer_link -->").replace('<a href="http://wordpress.org/">WordPress</a>',"WordPress<!-- pg_wp10_footer_link -->");a!==r&&await e.writeFile(s,a)}}}async function qe(e,t){const i=o.joinPaths(t,"wp-admin/edit.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i);if(n.includes("/* pg_wp10_post_title_edit */"))return;let s=n;const r='<strong><a href="<?php permalink_link(); ?>" rel="permalink"><?php the_title() ?></a></strong>';s.includes(r)&&(s=s.replace(r,'<strong><a href="post.php?action=edit&amp;post=<?php echo $id /* pg_wp10_post_title_edit */ ?>"><?php the_title() ?></a></strong>'));const a='<td><a href="<?php the_permalink(); ?>" rel="permalink">';s.includes(a)&&(s=s.replace(a,'<td><a href="post.php?action=edit&amp;post=<?php echo $id /* pg_wp10_post_title_edit */ ?>">'));const l=`<td><?php the_title() ?>
		...
		<?php if ('private' == $post->post_status) _e(' - <strong>Private</strong>'); ?></td>`)),s!==n&&await e.writeFile(i,s)}async function We(e,t){const i=o.joinPaths(t,"wp-admin/includes/screen.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i);if(!n.includes("self::$this->_help_sidebar"))return;const s=n.replace(/self::\$this->_help_sidebar/g,"$this->_help_sidebar");s!==n&&await e.writeFile(i,s)}async function Ge(e,t){const i=o.joinPaths(t,"wp-admin/plugins.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i);if(n.includes("/* pg_wp21_active_plugins_array */"))return;const s="$current = get_option('active_plugins');";if(!n.includes(s))return;const r=n.replace(s,s+`
	if (!is_array($current)) $current = array(); /* pg_wp21_active_plugins_array */`);r!==n&&await e.writeFile(i,r)}async function Me(e,t){const i=o.joinPaths(t,"wp-login.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i),s="AND user_pass = '$password'";if(!n.includes(s)||n.includes("pg_wp10_plain_or_md5"))return;let r=n.replace(s,"AND (user_pass = '$password' OR user_pass = MD5('$password')) /* pg_wp10_plain_or_md5 */");r=r.replace("$login->user_pass == $password","($login->user_pass == $password || $login->user_pass == md5($password))"),r!==n&&await e.writeFile(i,r)}async function Be(e,t){const i=o.joinPaths(t,"wp-includes");if(!e.isDir(i))return;const n=o.joinPaths(i,"version.php");e.fileExists(n)||await e.writeFile(n,"<?php $wp_version = '1.0';")}async function He(e,t){const i=o.joinPaths(t,"wp-blog-header.php");if(e.fileExists(i)){const s=e.readFileAsText(i),r=`$where .= ' AND (post_status = "publish"';`;s.includes(r)&&await e.writeFile(i,s.replace(r,`$where .= " AND (post_status = 'publish'";`))}const n=o.joinPaths(t,"wp-includes/vars.php");if(e.fileExists(n)){const s=e.readFileAsText(n),r="add_filter('all', 'wptexturize');";s.includes(r)&&await e.writeFile(n,s.replace(r,`// ${r} // Disabled by Playground: mangles SQL literals.`))}}async function Xe(e,t){const i=o.joinPaths(t,"wp-load.php");e.fileExists(i)||await e.writeFile(i,`<?php
if ( !defined('ABSPATH') ) {
	define('ABSPATH', dirname(__FILE__) . '/');
}
require_once(ABSPATH . 'wp-config.php');
`)}async function je(e,t){const i=o.joinPaths(t,"wp-settings.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i);let s=n;s=s.replace(/if\s*\(\s*!extension_loaded\('mysql'\)\s*\)\s*\n\s*die/,`if ( false ) // Patched for SQLite
...
do_action('init');`)}async function Ke(e,t){const i=o.joinPaths(t,"wp-includes/functions.php");if(!e.fileExists(i))return;let n=e.readFileAsText(i),s=!1;n.includes("$all_options->{$option->option_name}")&&!n.includes("$all_options = new stdClass")&&(n=n.replace("foreach ($options as $option) {",`$all_options = new stdClass;
	foreach ($options as $option) {`),s=!0),s&&await e.writeFile(i,n)}async function Qe(e,t){const i=o.joinPaths(t,"wp-admin/install.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i);let s=n;const r=o.joinPaths(t,"wp-admin");s=s.replace(/'\.\.\/(wp-config\.php)'/g,`'${t}/$1'`).replace(/'\.\.\/(wp-load\.php)'/g,`'${t}/$1'`).replace(/'\.\/(upgrade-functions\.php)'/g,`'${r}/$1'`).replace(/'(upgrade-functions\.php)'/g,`'${r}/$1'`).replace(/'\.\/(includes\/upgrade\.php)'/g,`'${r}/$1'`).replace(/'\.\.\/(wp-includes\/[^']+)'/g,`'${t}/$1'`),s=s.replace(/\$HTTP_GET_VARS/g,"$_GET").replace(/\$HTTP_POST_VARS/g,"$_POST"),s!==n&&await e.writeFile(i,s)}async function ze(e,t){const i=o.joinPaths(t,"wp-includes/wp-db.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i);let s=n;s.includes("isset($wpdb)")||(s=s.replace("$wpdb = new wpdb(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST);","if ( !isset($wpdb) ) { $wpdb = new wpdb(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST); }")),s.includes("db_connect")||(s=s.replace(/\$this->dbh\s*=\s*@mysql_connect\(\$dbhost\s*,\s*\$dbuser\s*,\s*\$dbpassword(?:\s*,\s*true)?\);/,'if (method_exists($this, "db_connect")) { $this->dbname = $dbname; $this->db_connect(); } else { $this->dbh = @mysql_connect($dbhost, $dbuser, $dbpassword); }')),s=Ze(s),s!==n&&await e.writeFile(i,s)}function Ze(e){const t=[];if(e.includes("function set_prefix")||t.push(`
	function set_prefix($prefix) {
		$this->prefix = $prefix;
...
`;return e.substring(0,i.index)+n+e.substring(i.index)}async function Je(e,t){const i=o.joinPaths(t,"wp-admin");if(e.isDir(i))for(const r of e.listFiles(i)){if(!r.endsWith(".php"))continue;const a=o.joinPaths(i,r),l=e.readFileAsText(a),_=H(l).replace(/ABSPATH\s*\.\s*'\/wp-/g,"ABSPATH . 'wp-");_!==l&&await e.writeFile(a,_)}const n=o.joinPaths(t,"wp-admin/index.php");if(e.fileExists(n)){let r=e.readFileAsText(n);r.includes("get_settings('siteurl')")&&(r=r.replace(/get_settings\('siteurl'\)\s*\.\s*'\/wp-admin\//g,"'"),await e.writeFile(n,r))}const s=o.joinPaths(t,"wp-admin/menu.php");if(e.fileExists(s)){const r=e.readFileAsText(s),a="file('./menu.txt')";r.includes(a)&&await e.writeFile(s,r.replace(a,"file(dirname(__FILE__) . '/menu.txt')"))}}async function et(e,t){const i=o.joinPaths(t,"wp-admin/admin-functions.php");if(!e.fileExists(i))return;const n=e.readFileAsText(i);if(!n.includes("function check_admin_referer()")||!n.includes("$_SERVER['HTTP_REFERER']"))return;const s=tt(n,"check_admin_referer",`
	do_action('check_admin_referer', '');
`);s!==n&&await e.writeFile(i,s)}function tt(e,t,i){const n=`function ${t}()`,s=e.indexOf(n);if(s===-1)return e;const r=e.indexOf("{",s+n.length);if(r===-1)return e;let a=1;for(let l=r+1;l<e.length;l++){const _=e[l];if(_==="{")a++;else if(_==="}"&&(a--,a===0))return e.substring(0,r+1)+i+e.substring(l)}return e}async function nt(e,t){const i=o.joinPaths(t,"wp-admin/index.php");if(e.fileExists(i)){const n=e.readFileAsText(i),s=n.replace(/AND post_date_gmt < '\$today'/,"");s!==n&&await e.writeFile(i,s)}await it(e,t)}async function it(e,t){const i=o.joinPaths(t,"wp-includes/rss-functions.php");if(!e.fileExists(i))return;let n=e.readFileAsText(i);!/^\s*error\s*\(/m.test(n)||/^function\s+error\s*\(/m.test(n)||(n=n.replace(/^(<\?php\s*)/,`$1
...
`),await e.writeFile(i,n))}async function st(e,t){const i=o.joinPaths(t,"wp-login.php");if(!e.fileExists(i))return;let n=e.readFileAsText(i),s=!1;for(const r of["log","pwd"]){const a=new RegExp(`(\\bname=(['"])${r}\\2)(?![^>]*data-1p-ignore)`);a.test(n)&&(n=n.replace(a,"$1 data-1p-ignore"),s=!0)}s&&await e.writeFile(i,n)}async function rt(e,t){const i=o.joinPaths(t,"wp-admin/admin.php");if(e.fileExists(i)){const s=e.readFileAsText(i);if(s.includes("auth_redirect()")){const r=`
...
if (defined('PLAYGROUND_AUTO_LOGIN_AS_USER')) {
	if (function_exists('is_user_logged_in') && is_user_logged_in()) {
		...
		${V("PLAYGROUND_AUTO_LOGIN_AS_USER")}
...
?>`;r!==s&&await e.writeFile(n,r)}}}async function at(e,t){const i=o.joinPaths(t,"wp-admin/admin-ajax.php");if(!e.fileExists(i))return;let n=e.readFileAsText(i);if(!n.includes("is_user_logged_in"))return;const s=`
// Playground: authenticate admin user for AJAX requests on WP < 2.8.
if (defined('PLAYGROUND_AUTO_LOGIN_AS_USER')) {
	${V("PLAYGROUND_AUTO_LOGIN_AS_USER")}
}
`;n=n.replace(/if\s*\(\s*!\s*is_user_logged_in\(\)\s*\)/,s+"if ( !is_user_logged_in() )"),await e.writeFile(i,n)}function V(e){return`
$_pg_user = null;
if (function_exists('wp_generate_auth_cookie')) {
...
`}async function ot(e,t){const i=b(e,t);if(i===null)return;const n=parseFloat(i);if(!Number.isFinite(n)||n>=3.3)return;const s=o.joinPaths(t,"wp-admin/includes/schema.php");if(!e.fileExists(s))return;const r=e.readFileAsText(s);/\$wp_queries\s*=\s*"CREATE TABLE/.test(r)&&!r.includes("function wp_get_db_schema")&&await lt(e,t,s,r)}async function lt(e,t,i,n){const s=n.match(/\$wp_queries\s*=\s*"CREATE TABLE/);if(!s||s.index===void 0)return;const r=s.index,a='";',l=n.indexOf(a,r);if(l===-1)return;const _=l+a.length,d=`function wp_get_db_schema( $scope = 'all', $blog_id = null ) {
	...
	${n.substring(r,_)}
	return $wp_queries;
}`,p=n.substring(0,r)+d+n.substring(_);await e.writeFile(i,p);const u=o.joinPaths(t,"wp-admin/includes/upgrade.php");if(e.fileExists(u)){const h=e.readFileAsText(u),E=h.replace(/(\$alterations\s*=\s*dbDelta\(\s*\$wp_queries\s*\))/g,"if ( function_exists('wp_get_db_schema') ) { $wp_queries = wp_get_db_schema(); } $1");E!==h&&await e.writeFile(u,E)}}const O="@playground-managed";function _t(){return`<?php
// ${O} — Playground-generated db.php.
...
`}async function ct(e,t){if(await me(e))return;let i=null;const n=o.joinPaths(e.documentRoot,"wp-includes/version.php");if(e.fileExists(n)){const r=e.readFileAsText(n).match(/\$wp_version\s*=\s*['"]([^'"]+)['"]/);r&&(i=r[1])}const s=i!==null&&parseFloat(i)<3.5;try{await e.run({code:`<?php
			...
			`,env:{DOCUMENT_ROOT:e.documentRoot,PLAYGROUND_SITE_URL:t||""}})}catch(r){$.logger.warn("Legacy WP post-install fixups failed (non-fatal):",r)}if(s)try{await e.run({code:`<?php
				$db_dir = getenv('DOCUMENT_ROOT') . '/wp-content/database/';
				if (!is_dir($db_dir)) { @mkdir($db_dir, 0777, true); }
				$db_path = $db_dir . '.ht.sqlite';
				$pdo = new PDO('sqlite:' . $db_path);
			...
			`,env:{DOCUMENT_ROOT:e.documentRoot,PLAYGROUND_SITE_URL:t||""}})}catch(r){$.logger.warn("Legacy WP PDO fallback failed (non-fatal):",r)}}const ut=`
...
if (PHP_MAJOR_VERSION < 7) {
	...
	ini_set('error_reporting', $level);
		...
		`}const m=512,F="/__tar-root__",ht=[40,181,47,253],$t=[80,75,3,4],N=new TextDecoder;function Y(e){return ht.every((t,i)=>e[i]===t)}function gt(e){return $t.every((t,i)=>e[i]===t)}function K(e){const t=String(e);if(t.includes("\\"))throw new Error(`Unsafe tar entry (backslash path): ${e}`);if(t.startsWith("/"))throw new Error(`Unsafe tar entry (absolute path): ${e}`);const i=o.normalizePath(t);if(!i)return"";const n=o.resolvePathUnder(i,F);if(!n)throw new Error(`Unsafe tar entry (path traversal): ${e}`);return n.slice(F.length+1)}function Q(e,t,i){const n=e.subarray(t,t+i);let s="";for(const r of n){if(r===0||r===32){if(s)break;continue}s+=String.fromCharCode(r)}return s?Number.parseInt(s,8):0}function C(e,t,i){let n=t;const s=t+i;for(;n<s&&e[n]!==0;)n+=1;return N.decode(e.subarray(t,n))}function q(e){for(let t=0;t<e.length;t+=1)if(e[t]!==0)return!1;return!0}function mt(e,t){const i=Q(e,148,8);let n=0;for(let s=0;s<m;s+=1)n+=s>=148&&s<156?32:e[s];if(i!==n)throw new Error(`Malformed tar stream: invalid header checksum for ${t||"unknown"}`)}class z{constructor({onEntry:t}={}){this.leftover=new Uint8Array(0),this.state="header",this.entry=null,this.dataChunks=[],this.dataFilled=0,this.padRemaining=0,this.pendingLongName=null,this.pendingPax=null,this.zeroBlocks=0,this.maxBuffered=0,this.fileCount=0,this.dirCount=0,this.phpCount=0,this.bytesWritten=0,this.onEntry=t??(()=>{})}push(t){if(t!=null&&t.length)if(this.leftover.length===0)this.leftover=t;else{const i=new Uint8Array(this.leftover.length+t.length);i.set(this.leftover,0),i.set(t,this.leftover.length),this.leftover=i}this.track(),this.drain()}end(){var t,i,n,s;if(this.state==="data"&&this.dataFilled<(((t=this.entry)==null?void 0:t.size)??0))throw new Error(`Truncated tar stream: entry ${(i=this.entry)==null?void 0:i.name} expected ${(n=this.entry)==null?void 0:n.size} bytes, got ${this.dataFilled}`);if(this.state==="pad"&&this.padRemaining>0)throw new Error(`Truncated tar stream: entry ${((s=this.entry)==null?void 0:s.name)??"unknown"} missing ${this.padRemaining} padding bytes`);if(this.state==="header"&&this.leftover.length>0)throw new Error(`Truncated tar stream: incomplete header (${this.leftover.length} bytes)`);if(this.pendingLongName!==null)throw new Error("Malformed tar stream: GNU longlink entry was not followed by a file entry");if(this.pendingPax!==null)throw new Error("Malformed tar stream: PAX header was not followed by a file entry");if(this.zeroBlocks<2)throw new Error("Truncated tar stream: missing end-of-archive marker");return this.stats()}stats(){return{fileCount:this.fileCount,dirCount:this.dirCount,phpCount:this.phpCount,bytesWritten:this.bytesWritten,maxBuffered:this.maxBuffered}}track(t=0){const i=this.leftover.length+this.dataFilled+t;i>this.maxBuffered&&(this.maxBuffered=i)}drain(){let t=!0;for(;t;){if(t=!1,this.state==="header"){if(this.leftover.length<m)break;const i=this.leftover.subarray(0,m);if(this.leftover=this.leftover.subarray(m),q(i)){this.zeroBlocks+=1,this.zeroBlocks>=2&&(this.state="done"),t=!0;continue}this.zeroBlocks=0;const n=Q(i,124,12),s=String.fromCharCode(i[156])||"0",r=C(i,0,100),a=C(i,345,155);if(mt(i,r),!Number.isFinite(n)||n<0)throw new Error(`Malformed tar stream: invalid size for ${r}`);this.entry={name:r,prefix:a,size:n,typeflag:s,isLongLink:s==="L",isPaxHeader:s==="x"},this.dataChunks=[],this.dataFilled=0,n>0?this.state="data":this.finishEntry(),t=!0;continue}if(this.state==="data"){const i=this.entry.size-this.dataFilled;if(i<=0){this.finishEntry();continue}if(this.leftover.length===0)break;const n=Math.min(i,this.leftover.length);this.dataChunks.push(this.leftover.subarray(0,n)),this.dataFilled+=n,this.leftover=this.leftover.subarray(n),this.track(),this.dataFilled===this.entry.size&&this.finishEntry(),t=!0;continue}if(this.state==="pad"){if(this.padRemaining===0){this.state="header",t=!0;continue}if(this.leftover.length===0)break;const i=Math.min(this.padRemaining,this.leftover.length);this.leftover=this.leftover.subarray(i),this.padRemaining-=i,t=!0;continue}if(this.state==="done"){if(this.leftover.length===0)break;if(!q(this.leftover))throw new Error("Malformed tar stream: non-zero data after end-of-archive marker");this.leftover=new Uint8Array(0),t=!0}}}concatData(){const t=new Uint8Array(this.dataFilled);let i=0;for(const n of this.dataChunks)t.set(n,i),i+=n.length;return t}finishEntry(){var l;const t=this.entry,i=this.concatData();this.dataChunks=[];const n=t.size%m;if(this.padRemaining=n===0?0:m-n,this.state=this.padRemaining>0?"pad":"header",this.dataFilled=0,this.entry=null,t.isLongLink){this.pendingLongName=N.decode(i).replace(/\0.*$/,"");return}if(t.isPaxHeader){this.pendingPax=wt(i);return}const s=((l=this.pendingPax)==null?void 0:l.path)??this.pendingLongName??(t.prefix?`${t.prefix}/${t.name}`:t.name);this.pendingLongName=null,this.pendingPax=null;const r=t.typeflag==="5"||s.endsWith("/"),a=K(s);if(a){if(r){this.dirCount+=1,this.onEntry({type:"dir",path:a});return}if(t.typeflag!=="0"&&t.typeflag!=="\0")throw new Error(`Unsupported tar entry type "${t.typeflag}" for ${s}`);this.fileCount+=1,this.bytesWritten+=i.length,a.endsWith(".php")&&(this.phpCount+=1),this.onEntry({type:"file",path:a,data:i})}}}async function Z(e,t){const i=Tt(e);if(typeof DecompressionStream<"u")try{const n=new DecompressionStream(t);return i.pipeThrough(n)}catch{}if(t==="zstd"){const{ZSTDDecoder:n}=await Promise.resolve().then(()=>require("./zstddec-stream.modern-D_CFiovn.cjs")),s=new n;await s.init();const r=await Et(e),a=s.decodeStreaming(r);let l=!1;return new ReadableStream({pull(_){if(l){_.close();return}try{for(;;){const{value:c,done:d}=a.next();if(d){l=!0,_.close();return}if(c.length!==0){_.enqueue(c);return}}}catch(c){l=!0,_.error(c)}},cancel(){var _;l=!0,(_=a.return)==null||_.call(a,void 0)}})}throw new Error(`No streaming decoder available for codec "${t}".`)}async function J(e,t,i,n={}){const{onProgress:s=()=>{},overwriteFiles:r=!0}=n,a=String(i).replace(/\/+$/,"")||"/",l=new Set,_=p=>{if(!p||l.has(p))return;t.mkdirTree(p);let u=p;for(;u&&!l.has(u);){l.add(u);const h=o.dirname(u);u=h&&h!==u?h:null}},c=new z({onEntry:p=>{var h;const u=o.joinPaths(a,p.path);if(p.type==="dir"){_(u);return}_(o.dirname(u)),!(!r&&((h=t.fileExists)!=null&&h.call(t,u)))&&(t.writeFile(u,p.data),c.fileCount%1e3===0&&s({fileCount:c.fileCount,bytes:c.bytesWritten}))}});_(a);const d=e.getReader();for(;;){const{done:p,value:u}=await d.read();if(p)break;u&&c.push(u)}return c.end()}function Tt(e){return ee(e)?e:new Blob([e]).stream()}async function Et(e){if(!ee(e))return[e];const t=e.getReader(),i=[];try{for(;;){const{done:n,value:s}=await t.read();if(n)break;i.push(s)}return i}catch(n){throw await t.cancel(n).catch(()=>{}),n}finally{t.releaseLock()}}function ee(e){return typeof ReadableStream<"u"&&e instanceof ReadableStream}function wt(e){const t=N.decode(e),i={};let n=0;for(;n<t.length;){const s=t.indexOf(" ",n);if(s===-1)throw new Error("Malformed tar stream: invalid PAX record length");const r=Number.parseInt(t.slice(n,s),10);if(!Number.isFinite(r)||r<=0)throw new Error("Malformed tar stream: invalid PAX record length");const a=n+r;if(a>t.length||t[a-1]!==`
`)throw new Error("Malformed tar stream: truncated PAX record");const l=t.slice(s+1,a-1),_=l.indexOf("=");if(_===-1)throw new Error("Malformed tar stream: invalid PAX record");i[l.slice(0,_)]=l.slice(_+1),n=a}return i}const te=`<?php
...
`;async function ne(e,t){const i=o.joinPaths(t,"wp-config.php");!e.fileExists(i)&&e.fileExists(o.joinPaths(t,"wp-config-sample.php"))&&await e.writeFile(i,await e.readFileAsBuffer(o.joinPaths(t,"wp-config-sample.php"))),e.fileExists(i)&&await yt(e,i,{DB_NAME:"wordpress"})}async function Lt(e,t,i){const n=o.phpVars({wpConfigPath:t,constants:i});if((await e.run({code:`${te}
		...
		`})).errors.length>0)throw new Error("Failed to rewrite constants in wp-config.php.")}async function yt(e,t,i){const n=Object.keys(i),s=o.phpVars({wpConfigPath:t,constantNames:n}),r=await e.run({code:`${te}
		...
		`});if(r.errors.length>0)throw new Error("Failed to check wp-config.php for constants.");let a;try{a=JSON.parse(r.text)}catch{throw new Error(`Failed to parse wp-config.php constant check output: ${r.text}`)}for(const l of a)await e.defineConstant(l,i[l])}async function y(e,{usesSqlite:t,hasCustomDatabasePath:i}){const n=await e.getPrimaryPhp();if(n.isFile("/internal/shared/preload/0-sqlite.php"))return;const s=o.joinPaths(e.documentRoot,"wp-content/mu-plugins/sqlite-database-integration");if(!n.isDir(s)&&!t&&!i&&!bt(n))throw new Error("Error connecting to the MySQL database.")}function bt(e){const t=o.joinPaths(e.documentRoot,"wp-config.php");if(!e.isFile(t))return!1;const i=e.readFileAsText(t),n=i.match(/define\s*\(\s*['"]DB_NAME['"]\s*,\s*['"]([^'"]*)['"]/),s=i.match(/define\s*\(\s*['"]DB_USER['"]\s*,\s*['"]([^'"]*)['"]/);return!n||!s?!1:n[1]!=="database_name_here"&&s[1]!=="username_here"}const ie=["fsockopen","pfsockopen","curl_init","curl_exec","curl_multi_exec","mail"];async function Pt(e,t){var r,a;if(!f.isLegacyPHPVersion(t.phpVersion))return;const i=(((r=t.phpIniEntries)==null?void 0:r.disable_functions)??"").split(",").map(l=>l.trim()).filter(l=>l),s={disable_functions:Array.from(new Set([...i,...ie])).join(","),allow_url_fopen:"0"};(a=t.phpIniEntries)!=null&&a["date.timezone"]||(s["date.timezone"]="UTC"),await f.setPhpIniEntries(e,s)}async function Ot(e,t){var a,l;const i=await e.getPrimaryPhp();if((a=t.hooks)!=null&&a.beforeWordPressFiles&&await t.hooks.beforeWordPressFiles(i),t.wordPressZip&&await U(i,await t.wordPressZip,{expectedFileCount:t.wordPressBundleFileCount}),t.constants)for(const _ in t.constants)i.defineConstant(_,t.constants[_]);i.defineConstant("WP_HOME",t.siteUrl),i.defineConstant("WP_SITEURL",t.siteUrl),await Nt(i,e.documentRoot),await ve(i,e.documentRoot),(l=t.hooks)!=null&&l.beforeDatabaseSetup&&await t.hooks.beforeDatabaseSetup(i);let n=!1;t.sqliteIntegrationPluginZip&&(n=!0,await A(i,await t.sqliteIntegrationPluginZip,{phpVersion:t.phpVersion}),await At(i,e.documentRoot));const s=t.wordpressInstallMode??"download-and-install",r=!!t.dataSqlPath;return(s==="download-and-install"||s==="install-from-existing-files"||s==="install-from-existing-files-if-needed")&&(await y(e,{usesSqlite:n,hasCustomDatabasePath:r}),await Ut(i,e)),e}async function Nt(e,t){const i=o.joinPaths(t,"wp-config.php"),n=o.joinPaths(t,"wp-config-sample.php");!e.fileExists(i)&&e.fileExists(n)&&await e.writeFile(i,await e.readFileAsBuffer(n))}async function At(e,t){const i=o.joinPaths(t,"wp-content"),n=o.joinPaths(i,"db.php");e.isDir(i)&&!e.fileExists(n)&&await e.writeFile(n,_t())}async function Ut(e,t){try{await St(e)}catch(i){$.logger.warn("Legacy PHP WordPress installation error:",i)}await ct(e,t.absoluteUrl)}async function St(e){var r,a,l,_,c,d;const t=vt(e,e.documentRoot);if(t!==null){const p=parseFloat(t);if(p<2.1)return;if(p<=3){await xt(e);return}}const i={disable_functions:ie.join(","),allow_url_fopen:"0",error_reporting:String(xe)},n=await f.withPHPIniValues(e,i,async()=>await e.request({url:"/wp-admin/install.php?step=2",method:"POST",body:{language:"en",prefix:"wp_",weblog_title:"My WordPress Website",user_name:"admin",admin_password:"password",admin_password2:"password",Submit:"Install WordPress",pw_weak:"1",admin_email:"admin@localhost.com"}}));if(!(((r=n.text)==null?void 0:r.includes("Success"))||((a=n.text)==null?void 0:a.includes("successful"))||((l=n.text)==null?void 0:l.includes("Finished"))||((_=n.text)==null?void 0:_.includes("Already Installed"))||((c=n.text)==null?void 0:c.includes("already have WordPress installed"))||!1))throw new Error(`Failed to install WordPress – installer responded with "${(d=n.text)==null?void 0:d.substring(0,100)}"`);await Rt(e)}async function Rt(e){try{(await e.run({code:`<?php
				$db_dir = getenv('DOCUMENT_ROOT') . '/wp-content/database/';
				$db_path = $db_dir . '.ht.sqlite';
			...
			`,env:{DOCUMENT_ROOT:e.documentRoot}})).text!=="1"&&$.logger.warn("Failed to default to pretty permalinks after WP install.")}catch{$.logger.warn("Failed to set pretty permalinks after WP install (non-fatal).")}}async function xt(e){try{await e.run({code:`<?php
				define('WP_INSTALLING', true);
				error_reporting(${P});
				ini_set('display_errors', '0');
				ob_start();
				...
				echo 'OK';
			`,env:{DOCUMENT_ROOT:e.documentRoot}})}catch(t){$.logger.warn("runDbDeltaOnly failed (non-fatal):",t)}}function vt(e,t){const i=o.joinPaths(t,"wp-includes/version.php");if(!e.fileExists(i))return null;const s=e.readFileAsText(i).match(/\$wp_version\s*=\s*['"]([^'"]+)['"]/);return s?s[1]:null}async function It(e){const t=await re(e);return await se(t,e),t}async function se(e,t){var a,l,_,c,d,p,u,h,L,E,S,R,x,v,I,k,D;if(f.isLegacyPHPVersion(t.phpVersion))return Ot(e,t);(a=t.onProgress)==null||a.call(t,"Creating PHP runtime");const i=await e.getPrimaryPhp();if((l=t.hooks)!=null&&l.beforeWordPressFiles&&((_=t.onProgress)==null||_.call(t,"Mounting WordPress files"),await t.hooks.beforeWordPressFiles(i)),t.wordPressZip&&((c=t.onProgress)==null||c.call(t,"Extracting WordPress files"),await U(i,await t.wordPressZip,{expectedFileCount:t.wordPressBundleFileCount})),t.constants){(d=t.onProgress)==null||d.call(t,"Defining WordPress constants");for(const g in t.constants)i.defineConstant(g,t.constants[g])}t.dataSqlPath&&(i.defineConstant("DB_DIR",o.dirname(t.dataSqlPath)),i.defineConstant("DB_FILE",o.basename(t.dataSqlPath))),i.defineConstant("WP_HOME",t.siteUrl),i.defineConstant("WP_SITEURL",t.siteUrl),(p=t.onProgress)==null||p.call(t,"Starting PHP"),await ne(i,e.documentRoot),(u=t.hooks)!=null&&u.beforeDatabaseSetup&&((h=t.onProgress)==null||h.call(t,"Preparing database files"),await t.hooks.beforeDatabaseSetup(i));let n=!1;t.sqliteIntegrationPluginZip&&(n=!0,(L=t.onProgress)==null||L.call(t,"Installing SQLite integration"),await A(i,await t.sqliteIntegrationPluginZip,{phpVersion:t.phpVersion}),await Re(i,e.documentRoot));const s=t.wordpressInstallMode??"download-and-install",r=!!t.dataSqlPath;if(["download-and-install","install-from-existing-files"].includes(s)){(E=t.onProgress)==null||E.call(t,"Checking database prerequisites"),await y(e,{usesSqlite:n,hasCustomDatabasePath:r});try{(S=t.onProgress)==null||S.call(t,"Running WordPress installer"),await W(i)}catch(g){throw r||await w(e),g}r||((R=t.onProgress)==null||R.call(t,"Validating database connection"),await w(e))}else if(s==="install-from-existing-files-if-needed"){if((x=t.onProgress)==null||x.call(t,"Checking database prerequisites"),await y(e,{usesSqlite:n,hasCustomDatabasePath:r}),(v=t.onProgress)==null||v.call(t,"Checking existing WordPress installation"),!await ae(i))try{(I=t.onProgress)==null||I.call(t,"Running WordPress installer"),await W(i)}catch(g){throw r||await w(e),g}r||((k=t.onProgress)==null||k.call(t,"Validating database connection"),await w(e))}return(D=t.onProgress)==null||D.call(t,"WordPress boot complete"),e}async function w(e){const t=await e.getPrimaryPhp();if(await Dt(t))return;if(t.isFile("/internal/shared/preload/0-sqlite.php"))throw new Error("Error connecting to the SQLite database.");const n=o.joinPaths(e.documentRoot,"wp-content/mu-plugins/sqlite-database-integration");throw t.isDir(n)?new Error("Error connecting to the SQLite database."):new Error("Error connecting to the MySQL database.")}async function re(e){kt(e);const t=e.spawnHandler??f.sandboxedSpawnHandlerFactory;async function i(s,r=!1){const a=await e.createPhpRuntime(r),l=new f.PHP(a);if(e.sapiName&&l.setSapiName(e.sapiName),s&&(l.requestHandler=s),e.phpIniEntries&&await f.setPhpIniEntries(l,e.phpIniEntries),await Pt(l,{phpVersion:e.phpVersion,phpIniEntries:e.phpIniEntries}),l.defineConstant("WP_SQLITE_AST_DRIVER",!0),e.constants)for(const _ in e.constants)l.defineConstant(_,e.constants[_]);return r&&!l.isFile("/internal/.boot-files-written")&&(await ce(l,{phpVersion:e.phpVersion}),await f.writeFiles(l,"/",e.createFiles||{}),await ue(l,o.joinPaths(new URL(e.siteUrl).pathname,"phpinfo.php")),await f.writeFiles(l,"/internal",{".boot-files-written":""})),t&&await l.setSpawnHandler(t(s?()=>s.instanceManager.acquirePHPInstance():void 0)),l.enableRuntimeRotation({recreateRuntime:e.createPhpRuntime,maxRequests:400}),e.onPHPInstanceCreated&&await e.onPHPInstanceCreated(l,{isPrimary:r}),l}const n=new f.PHPRequestHandler({documentRoot:e.documentRoot||"/wordpress",absoluteUrl:e.siteUrl,rewriteRules:_e,pathAliases:e.pathAliases,getFileNotFoundAction:e.getFileNotFoundAction??oe,cookieStore:e.cookieStore,php:e.maxPhpInstances===1?await i(void 0,!0):void 0,phpFactory:e.maxPhpInstances!==1?async({isPrimary:s})=>i(n,s):void 0,maxPhpInstances:e.maxPhpInstances});return n}function kt(e){"SQLITE_JOURNAL_MODE"in(e.constants??{})||(e.constants={...e.constants,SQLITE_JOURNAL_MODE:"DELETE"})}async function ae(e){return(await e.run({code:`<?php
			ob_start();
			$wp_load = getenv('DOCUMENT_ROOT') . '/wp-load.php';
			...
			ob_end_flush();
		`,env:{DOCUMENT_ROOT:e.documentRoot}})).text==="1"}async function W(e){var n;const t=await f.withPHPIniValues(e,{disable_functions:"fsockopen",allow_url_fopen:"0"},async()=>await e.request({url:"/wp-admin/install.php?step=2",method:"POST",body:{language:"en",prefix:"wp_",weblog_title:"My WordPress Website",user_name:"admin",admin_password:"password",admin_password2:"password",Submit:"Install WordPress",pw_weak:"1",admin_email:"admin@localhost.com"}}));if(!await ae(e))throw new Error(`Failed to install WordPress – installer responded with "${(n=t.text)==null?void 0:n.substring(0,100)}"`);(await e.run({code:`<?php
			ob_start();
			$wp_load = getenv('DOCUMENT_ROOT') . '/wp-load.php';
			if (!file_exists($wp_load)) {
				echo '0';
			...
			ob_end_flush();
		`,env:{DOCUMENT_ROOT:e.documentRoot}})).text!=="1"&&$.logger.warn("Failed to default to pretty permalinks after WP install.")}function oe(e){return{type:"internal-redirect",uri:"/index.php"}}async function Dt(e){return(await e.run({code:`<?php
			ob_start();
			$wp_load = getenv('DOCUMENT_ROOT') . '/wp-load.php';
			if (!file_exists($wp_load)) {
				echo '-1';
			...
			ob_end_flush();
		`,env:{DOCUMENT_ROOT:e.documentRoot}})).text==="1"}async function Ft(e){const{php:t,reap:i}=await e.instanceManager.acquirePHPInstance();try{const s=(await t.run({code:`<?php
				require '${e.documentRoot}/wp-includes/version.php';
				echo $wp_version;
			`})).text;if(!s)throw new Error("Unable to read loaded WordPress version.");return le(s)}finally{i()}}function le(e){if(/-(alpha|beta|RC)\d*-\d+$/.test(e))return"trunk";if(/-(beta|RC)\d*$/.test(e))return"beta";const n=e.match(/^(\d+\.\d+)(?:\.\d+)?$/);return n!==null?n[1]:e}const Ct="https://api.wordpress.org/core/stable-check/1.0/";let G;async function qt(){G??(G=T.createMemoizedFetch(globalThis.fetch));const e=await G(Ct);if(!e.ok)throw new Error("Could not load the WordPress release catalog: "+`${e.status} ${e.statusText}`.trim());const t=await e.json();if(!t||typeof t!="object"||Array.isArray(t))throw new Error("The WordPress release catalog returned invalid data.");return Object.keys(t)}const _e=[{match:new RegExp("^(/[_0-9a-zA-Z-]+)?(/wp-(content|admin|includes)/.*)"),replacement:"$2"}];async function ce(e,t={}){if(f.isLegacyPHPVersion(t.phpVersion))return fe(e);await e.mkdir("/internal/shared/mu-plugins"),await e.writeFile("/internal/shared/preload/env.php",`<?php
    ...
    `),await e.writeFile("/internal/shared/mu-plugins/1-auto-login.php",`<?php
		...
		})();`)}async function ue(e,t="/phpinfo.php"){await e.writeFile("/internal/shared/preload/phpinfo.php",`<?php
    // Render PHPInfo if the requested page is /phpinfo.php
    if ( isset($_SERVER['REQUEST_URI']) && ${o.phpVar(t)} === $_SERVER['REQUEST_URI'] ) {
        phpinfo();
        exit;
    }
    `)}async function A(e,t,i={}){if(f.isLegacyPHPVersion(i.phpVersion))return dt(e,t,i);await e.isDir("/tmp/sqlite-database-integration")&&await e.rmdir("/tmp/sqlite-database-integration",{recursive:!0}),await e.mkdir("/tmp/sqlite-database-integration"),await T.unzipFile(e,t,"/tmp/sqlite-database-integration");const n="/internal/shared/sqlite-database-integration",s=`/tmp/sqlite-database-integration/${(await e.listFiles("/tmp/sqlite-database-integration"))[0]}`;await e.mv(s,n);const r=o.joinPaths(n,"wp-includes/sqlite/class-wp-sqlite-db.php");if(await e.fileExists(r)){const p=await e.readFileAsText(r),u=p.replace("private $allow_unsafe_unquoted_parameters","protected $allow_unsafe_unquoted_parameters");u!==p&&await e.writeFile(r,u)}await e.defineConstant("SQLITE_MAIN_FILE","1");const l=(await e.readFileAsText(o.joinPaths(n,"db.copy"))).replace("'{SQLITE_IMPLEMENTATION_FOLDER_PATH}'",o.phpVar(n)).replace("'{SQLITE_PLUGIN}'",o.phpVar(o.joinPaths(n,"load.php"))),_=o.joinPaths(await e.documentRoot,"wp-content/db.php"),c=`
		// Do not preload this if WordPress comes with a custom db.php file.
		if(file_exists(${o.phpVar(_)})) {
			return;
		}
		`,d="/internal/shared/mu-plugins/sqlite-database-integration.php";await e.writeFile(d,`<?php${c}?>`+l),await e.writeFile("/internal/shared/preload/0-sqlite.php",Wt(c,d)),await e.writeFile("/internal/shared/mu-plugins/sqlite-test.php",`<?php
		global $wpdb;
		if(!($wpdb instanceof WP_SQLite_DB)) {
			var_dump(isset($wpdb));
			die("SQLite integration not loaded " . get_class($wpdb));
		}
		`)}function Wt(e,t){return`<?php
	if(!function_exists('mysqli_connect')) {
	...
	${B(`require_once ${o.phpVar(t)};`)}
		...
		`}async function U(e,t,i={}){e.mkdir("/tmp/unzipped-wordpress");const n=new Uint8Array(await t.slice(0,4).arrayBuffer());if(Y(n)){const a=await Z(t.stream(),"zstd"),l=await J(a,e,"/tmp/unzipped-wordpress");if(i.expectedFileCount!=null&&l.fileCount!==i.expectedFileCount)throw new de(l.fileCount,i.expectedFileCount)}else await T.unzipFile(e,t,"/tmp/unzipped-wordpress");e.fileExists("/tmp/unzipped-wordpress/wordpress.zip")&&await T.unzipFile(e,"/tmp/unzipped-wordpress/wordpress.zip","/tmp/unzipped-wordpress");let s=e.fileExists("/tmp/unzipped-wordpress/wordpress")?"/tmp/unzipped-wordpress/wordpress":e.fileExists("/tmp/unzipped-wordpress/build")?"/tmp/unzipped-wordpress/build":"/tmp/unzipped-wordpress";if(!e.fileExists(o.joinPaths(s,"wp-config-sample.php"))){const a=e.listFiles(s);if(a.length){const l=a[0];e.fileExists(o.joinPaths(s,l,"wp-config-sample.php"))&&(s=o.joinPaths(s,l))}}const r=(a,l,_)=>{if(_.isDir(a)&&_.isDir(l))for(const c of _.listFiles(a)){const d=o.joinPaths(a,c),p=o.joinPaths(l,c);r(d,p,_)}else{if(_.fileExists(l)){const c=a.replace(/^\/tmp\/unzipped-wordpress\//,"/");$.logger.warn(`Cannot unzip WordPress files at ${l}: ${c} already exists.`);return}_.mv(a,l)}};r(s,e.documentRoot,e),e.fileExists(s)&&e.rmdir(s,{recursive:!0}),!e.fileExists(o.joinPaths(e.documentRoot,"wp-config.php"))&&e.fileExists(o.joinPaths(e.documentRoot,"wp-config-sample.php"))&&e.writeFile(o.joinPaths(e.documentRoot,"wp-config.php"),e.readFileAsText(o.joinPaths(e.documentRoot,"/wp-config-sample.php")))}class de extends Error{constructor(t,i){super(`WordPress core bundle file-count parity check failed: extracted ${t} files, expected ${i}. The download may be truncated or corrupt.`),this.name="WordPressBundleFileCountMismatchError"}}const Gt=T.createMemoizedFetch(fetch),Mt="https://github.com/WordPress/WordPress/archive/refs/heads/master.zip";async function Bt(e="latest"){if(e===null)e="latest";else if(e.startsWith("https://")||e.startsWith("http://")){const n=await crypto.subtle.digest("SHA-1",new TextEncoder().encode(e)),s=Array.from(new Uint8Array(n)).map(r=>r.toString(16).padStart(2,"0")).join("");return{releaseUrl:e,version:"custom-"+s.substring(0,8),source:"inferred"}}else if(e==="trunk"||e==="nightly"){const n=new Date().toISOString().split("T")[0];return{releaseUrl:`${Mt}?ts=${n}`,version:"trunk",source:"inferred"}}let i=await(await Gt("https://api.wordpress.org/core/version-check/1.7/?channel=beta")).json();i=i.offers.filter(n=>n.response==="autoupdate");for(const n of i){if(e==="beta"&&(n.version.includes("beta")||n.version.includes("RC")))return{releaseUrl:n.download,version:n.version,source:"api"};if(e==="latest"&&!n.version.includes("beta")&&!n.version.includes("RC"))return{releaseUrl:n.download,version:n.version,source:"api"};if(n.version.substring(0,e.length)===e)return{releaseUrl:n.download,version:n.version,source:"api"}}return e.match(/^\d+\.\d+\.0$/)&&(e=e.split(".").slice(0,2).join(".")),{releaseUrl:`https://wordpress.org/wordpress-${e}.zip`,version:e,source:"inferred"}}exports.PLAYGROUND_MANAGED_DB_PHP_MARKER=O;exports.StreamingTarParser=z;exports.WordPressBundleFileCountMismatchError=de;exports.bootRequestHandler=re;exports.bootWordPress=se;exports.bootWordPressAndRequestHandler=It;exports.createDecodedTarStream=Z;exports.defineWpConfigConstants=Lt;exports.ensureWpConfig=ne;exports.extractTarStreamToPhp=J;exports.getFileNotFoundActionForWordPress=oe;exports.getLoadedWordPressVersion=Ft;exports.getWordPressStableVersions=qt;exports.isZipBundle=gt;exports.isZstdBundle=Y;exports.preloadPhpInfoRoute=ue;exports.preloadSqliteIntegration=A;exports.resolveWordPressRelease=Bt;exports.sanitizeTarPath=K;exports.setupPlatformLevelMuPlugins=ce;exports.unzipWordPress=U;exports.versionStringToLoadedWordPressVersion=le;exports.wordPressRewriteRules=_e;
