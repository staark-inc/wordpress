import { phpVars as m } from "@php-wasm/util";
function R(s = fetch) {
  const e = {};
  return async function(t, c) {
    if (!e[t]) {
      e[t] = {
        responsePromise: s(t, c),
        async nextResponse() {
          const n = await e[t].responsePromise, [r, a] = e[t].unlockedBodyStream.tee();
          return e[t].unlockedBodyStream = r, new Response(a, {
            status: n.status,
      ...
      const o = await e[t].responsePromise;
      e[t].unlockedBodyStream = o.body;
    }
    return e[t].nextResponse();
  };
}
const x = "8.3", y = 100, h = 400 * 1024, P = "PLAYGROUND_UNZIP_PROGRESS:", U = async (s, e, i, t = !0, c) => {
  const o = `/tmp/file-${Math.random()}.zip`;
  let n = !1;
  try {
    if (e instanceof File) {
      const $ = e;
      e = o, n = !0, await s.writeFile(
        e,
    ...
    const a = {
      code: `<?php
		...
		`,
      env: {
        PLAYGROUND_UNZIP_ZIP_PATH: e,
        PLAYGROUND_UNZIP_EXTRACT_TO_PATH: i,
        PLAYGROUND_UNZIP_OVERWRITE_FILES: t ? "1" : "0",
        PLAYGROUND_UNZIP_REPORT_PROGRESS: c ? "1" : "0",
        ...
        PLAYGROUND_UNZIP_LINE_PREFIX: P
      }
    };
    c ? await _(s, a, c) : await s.run(a);
  } finally {
    if (n)
      try {
        await s.fileExists(o) && await s.unlink(o);
      } catch {
      }
  }
};
async function _(s, e, i) {
  const t = await s.runStream(e), c = t.stderrText, o = t.stdout.getReader(), n = new TextDecoder();
  let r = "", a;
  const $ = (l) => {
    if (l.startsWith(P))
      try {
  ...
  try {
    for (; ; ) {
      ...
      r += n.decode(p, { stream: !l });
      let d = r.indexOf(`
`);
      for (; d !== -1; )
        $(r.slice(0, d)), r = r.slice(d + 1), d = r.indexOf(`
`);
      if (l)
        break;
    }
...
const I = async (s, e) => {
  const i = `/tmp/file${Math.random()}.zip`;
  try {
    const t = m({
      directoryPath: e,
      outputPath: i
    ...
    return await s.run({
      code: `<?php
			...
			`
    }), await s.readFileAsBuffer(i);
  } finally {
    try {
      await s.fileExists(i) && await s.unlink(i);
    } catch {
    }
  }
};
export {
  x as RecommendedPHPVersion,
  R as createMemoizedFetch,
  U as unzipFile,
  I as zipDirectory
};
