...
async function $(e) {
  ...
  try {
    ...
    const h = await P(c, i);
    if (o = await H(self, h), delete o.headers["x-frame-options"], o.headers["content-security-policy"]) {
...
async function W(e, t, r, n) {
  ...
  try {
    ...
    const i = new Headers(s.headers), h = ((c = i.get("x-cors-proxy-allowed-request-headers")) == null ? void 0 : c.split(",")) || [], f = h.includes("authorization") || h.includes("cookie"), l = i.get("content-type");
    ...
    g !== "GET" && g !== "HEAD" && (await A() ? p = d.body : p = await d.arrayBuffer()), p instanceof ReadableStream && new URL(r, import.meta.url).protocol === "http:" && (p = await new Response(p).arrayBuffer());
    const T = await w(s, {
      url: `${r}${s.url}`,
      headers: i,
      body: p,
