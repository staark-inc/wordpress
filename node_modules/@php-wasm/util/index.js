    ...
    if (this._running >= this.concurrency) {
      ...
      if (this.timeout !== void 0) {
        ...
        if (await Promise.race([
          ...
          throw this.queue.splice(this.queue.indexOf(r), 1), new b();
      } else
        await t;
    }
    this._running++;
...
function $(i) {
  if (i < 1024)
    return `${i} B`;
  const e = i / 1024 / 1024;
  return e >= 1 ? `${e.toFixed(1)} MB` : `${(i / 1024).toFixed(0)} KB`;
}
function C() {
  return B(36, "-_");
}
function Q(i) {
  return new TextDecoder().decode(k(i));
}
function k(i) {
  const e = atob(i), t = e.length, r = new Uint8Array(t);
  for (let s = 0; s < t; s++)
    r[s] = e.charCodeAt(s);
  return r;
}
function O(i) {
  return U(new TextEncoder().encode(i));
}
function U(i) {
  const t = [];
  for (let r = 0; r < i.length; r += 65536)
    t.push(String.fromCharCode(...i.subarray(r, r + 65536)));
  return btoa(t.join(""));
}
function q(i) {
...
function R(i) {
  return D(i.map((e) => new Uint8Array(e))).buffer;
}
export {
  b as AcquireTimeoutError,
  ...
  j as basename,
  R as concatArrayBuffers,
  D as concatUint8Arrays,
  S as createSpawnHandler,
  Q as decodeBase64ToString,
  k as decodeBase64ToUint8Array,
  W as dirname,
  O as encodeStringAsBase64,
  U as encodeUint8ArrayAsBase64,
  F as ensureAbsolutePath,
  $ as formatBytes,
  P as isParentOf,
  m as joinPaths,
  c as normalizePath,
  A as phpEventStdinTransfer,
  q as phpVar,
  L as phpVars,
  C as randomFilename,
  B as randomString,
  I as resolvePathUnder,
  N as sendmailSpawnHandler,
